...
async function ensureSchema() {
  ...
  // 로그인할 때마다 검색 필터(숙소유형/예약옵션/시설 등)를 매번 다시 고르는 게 번거롭다는 요청 -
  // 마지막으로 쓴 필터 조합을 계정에 저장해뒀다가 다음 로그인에 그대로 불러온다.
  await getPool().query(`
    ALTER TABLE users ADD COLUMN IF NOT EXISTS search_prefs JSONB;
  `);
}
